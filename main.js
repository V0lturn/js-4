...
function getLocalDate(date, isSeconds = false, isISO = false){
    if(!isISO){
        return isSeconds ? date.toLocaleString() : date.toLocaleString().slice(0, -3);
    }
    else{
        ...
        let seconds = date.getSeconds() < 9 ? `0${date.getSeconds()}` : date.getUTCSeconds();
        return isSeconds ? `${year}-${month}-${day}, ${hour}:${minutes}:${seconds}` : `${year}-${month}-${day}, ${hour}:${minutes}`;
    }
}
